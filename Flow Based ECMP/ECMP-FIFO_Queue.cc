...
class QueueMonitor
{
    ...
    public:
        ...
        double GetAvgQueueLen()
        {
            ...
            {
                length=m_qLens[i];
                sum_lengths+=length*(m_qTimes[i+1].GetSeconds()-m_qTimes[i].GetSeconds());
            }
...
int main (int argc, char *argv[])
{
    ...
    CommandLine cmd;
    cmd.AddValue ("QueueSize","QueueSize",QueueSize);
    ...
    PointToPointHelper pointToPointQueue;
    pointToPointQueue.SetQueue ("ns3::DropTailQueue", "MaxSize", StringValue ("1p"));
    pointToPointQueue.SetDeviceAttribute ("DataRate", StringValue( "5Mbps"));
    pointToPointQueue.SetChannelAttribute ("Delay", StringValue("2ms"));
    ...
    for(int i=0;i<nPath;i++)
    {
        ...
        group2Device[i]=pointToPointQueue.Install(group2[i]);
    }
    ...
    QueueDiscContainer tch_app[nPath];
    tch.SetRootQueueDisc ("ns3::FifoQueueDisc", "MaxSize", StringValue (QueueSize));
    QueueMonitor *qm=new QueueMonitor[nPath]();
    for(int i=0;i<nPath;i++)
    {
        qm[i]=QueueMonitor(Seconds(0.0));
        tch_app[i]=tch.Install (group2Device[i]);
        tch_app[i].Get(0)->TraceConnectWithoutContext ("PacketsInQueue", MakeCallback(&QueueMonitor::QueueChange,&qm[i]));
    ...
    for(int i=0;i<nFlow;i++)
    {
        OnOffHelper OnOffClient ("ns3::TcpSocketFactory", InetSocketAddress(interface_ip[nFlow+2*nPath].GetAddress(1),7999));
    ...
    stringstream ss;
    ss<<"Hw3-3/All-<"<<to_string(EcmpMode)<<">-<"<<to_string(nPath)<<">-<"<<to_string(nFlow)<<">-<"<<QueueSize<<">";
    string s;
    ss>>s;
    ...
    QueueDisc::Stats st[nPath];

    for(int i=0;i<nPath;i++)
    {
        stringstream tmp;
        tmp<<"Hw3-3/Q-<"<<to_string(EcmpMode)<<">-<"<<to_string(nPath)<<">-<"<<to_string(nFlow)<<">-<"<<QueueSize<<">-<"<<to_string(i)<<">.txt";
        string t;
        tmp>>t;
        qm[i].SaveQueueLen(t);
        st[i]=tch_app[i].Get(0)->GetStats();
    ...
    NS_LOG_UNCOND("The calculated Avg Queue Length for path 1 is "<<qm[0].GetAvgQueueLen()<<".");
    NS_LOG_UNCOND("The calculated Avg Queue Length for path 1 is "<<qm[1].GetAvgQueueLen()<<".");
    NS_LOG_UNCOND("The calculated Avg Queue Length for path 1 is "<<qm[2].GetAvgQueueLen()<<".");
    NS_LOG_INFO ("Done.");
    return 0;
}
