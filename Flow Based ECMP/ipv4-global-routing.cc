  ...
  static TypeId tid = TypeId ("ns3::Ipv4GlobalRouting")
    ...
    .SetGroupName ("Internet")
        .AddAttribute ("EcmpMode",
                   "xxxx; yyy",
                   UintegerValue(0),
                   MakeUintegerAccessor (&Ipv4GlobalRouting::m_ecmpMode),
...
Ptr<Ipv4Route>
Ipv4GlobalRouting::LookupGlobal (Ipv4Address dest,Ptr<const Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif)
{
      ...
      if (m_randomEcmpRouting)
        {
          ...
          uint8_t src_ip[4];
          header.GetSource().Serialize(src_ip);
 

          uint8_t dst_ip[4];
          header.GetDestination().Serialize(dst_ip);
          ...
          flow_id[2]=src_ip[2];
          flow_id[3]=src_ip[3];
          flow_id[5]=srcPort & 0xff;
          flow_id[4]=(srcPort>>8)&0xff;
          flow_id[6]=dst_ip[0];
          flow_id[7]=dst_ip[1];
          flow_id[8]=dst_ip[2];
          flow_id[9]=dst_ip[3];
          flow_id[11]=dstPort & 0xff;
          flow_id[10]=(dstPort>>8)&0xff;
          flow_id[12]=protocol;

          uint32_t hash_value;
          size_t buffer_size =13;
          hash_value=Hash32(flow_id,buffer_size);
          ...
          if(m_ecmpMode==1) //Modulo-N hash flow ID
          {
            selectIndex = hash_value % allRoutes.size();
          }
          if(m_ecmpMode==2) //Hash-Threshold
          {
            uint32_t len=0xffffffff/allRoutes.size();
            uint32_t num=hash_value/len;
            if(num>=allRoutes.size())
          ...
          if(m_ecmpMode==3) //HRW
          {
              ...
              for (int i = 0; i < count;i++)
              {
                  ...
                  uint32_t tmp = allRoutes[i]->GetInterface();
                  hrw_id[13]=(tmp>>24)&0xff;

                  hrw_id[14]=(tmp>>16)&0xff;

                  hrw_id[15]=(tmp>>8)&0xff;

                  hrw_id[16]=tmp&0xff;;
                  hash_value=Hash32(hrw_id,17);
                  if(hash_value>max){
                    hop=i;
                    max=hash_value;
                  }
...
{ 
  ...
  Ptr<Ipv4Route> rtentry = LookupGlobal (header.GetDestination (),p,header,0);
  if (rtentry != 0)
    {
